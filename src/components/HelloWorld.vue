    ...
    <el-container>
      <el-aside class="aside_button col-md-3">
        <div class="title col-md-12">
          <h3 class="col-md-12 blue_text">
            <i class="el-icon-s-help white_text"></i>Issue管理系统
          </h3>
        </div>
        <div class="registerPage col-md-12">
          <el-button type="primary" class="white_text" v-if="registerable">
            <router-link to="/registerpage" class="white_text"
              ><h5>
                <i class="el-icon-s-help white_text"></i>注册
              </h5></router-link
            >
          </el-button>
          <div class="block">
        ...
        <div class="changePage col-md-12">
          <router-link to="/changepage" v-if="changeable" class="white_text"
            ><h5>
              <i class="el-icon-s-tools white_text"></i>修改个人信息
            </h5></router-link
          >
        </div>
        <div class="createIssue col-md-12">
          <router-link to="/createissue" v-if="createissue" class="white_text"
            ><h5>
              <i class="el-icon-circle-plus white_text"></i>创建新Issue
            </h5></router-link
          >
        </div>
        <div class="issueReport col-md-12">
          <router-link to="/issueReport" v-if="issuereport" class="white_text"
            ><h5>
              <i class="el-icon-star-on white_text"></i>Issue报表
            </h5></router-link
          >
        </div>
      ...
      <div class="img_body col-md-9">
        ...
        <div class="welcome col-md-11">
          ...
          <h2 v-if="entersuccess" class="col-12 white_text">
            <i class="el-icon-user-solid white_text"></i>欢迎， {{ rName }}
          </h2>
        </div>
        <!-- 日历 -->
        <div class="left-wrap col-md-11" id="calendar">
          <el-calendar v-model="calendar_data"> </el-calendar>
        </div>
      </div>
    </el-container>
    <el-drawer
    ...
    submitForm(ruleForm) {
      this.$refs[ruleForm].validate((valid) => {
        if (valid) {
          ...
          this.$http
            .get("http://localhost:8080/user/selectbyid/" + this.ruleForm.rId)
            .then(function (res) {
